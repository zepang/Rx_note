...
|书签|可以收藏为书签|不能收藏为书签|
|缓存|能被缓存|不能被缓存|
|历史|参数保留在浏览器历史记录中|参数不保留|
|对数据长度限制|当发送数据时，GET 方法向 URL 添加数据；URL 的长度是受限制的（URL 的最大长度是 2048 个字符）|无限制|
|对数据类型限制|只允许 ASCII 字符|	没有限制。也允许二进制数据|
|安全性|与 POST 相比，GET 的安全性较差，因为所发送的数据是 URL 的一部分。在发送密码或其他敏感信息时绝不要使用 GET ！|POST 比 GET 更安全，因为参数不会被保存在浏览器历史或 web 服务器日志中。|

以上是常见的回答，建议使用下边的回答：

1. GET请求能够被缓存，POST不能被缓存
2. 刷新浏览器的时候，GET请求和参数都会再次被提交，POST的请求刷新的时候会提示表单将会重新被提交，并且不会保留参数
3. GET请求传递数据是会在URL上添加数据，相对于POST请求来说，安全性较差。涉及安全信息或者资源操作不能使用GET请求。
4. GET请求向URL添加数据，这个数字长度会因为浏览器原因有限制
